...
if (!function_exists('get_media_for_postid')) {
    function get_media_for_postid()
    {
        if (is_admin()) {
            ...
            $media_for_this_post = gt3_get_media_for_this_post($postid, $page);
            if (is_array($media_for_this_post) && count($media_for_this_post) > 0) {
                echo gt3_get_media_html($media_for_this_post, "small");
            } else {
                echo "no_items";
            }
        }
...
if (!function_exists('get_module_html')) {
    function get_module_html()
    {
        ...
        echo get_pb_module($module_name, $module_caption, gt3_get_unused_id(), "", $module_size, $size_caption);
