        ...
        function shortcode_promo_text($atts, $content = null)
        {
			...
			<div class="shortcode_promoblock '.$someClass.'">
			    <div class="promo_text_block">
                    ...
                    ' . (isset($additional_text) ? "<h2>" . $additional_text . "</h2>" : '') . '
                </div>
			        ' . ((strlen($button_link) > 0 && strlen($button_text) > 0) ? '<div class="promo_button_block"><a href="'.$button_link.'" class="promo_button">'.$button_text.'</a></div>' : '') . '
            </div>
            ';
...
$gt3_shortcodesUI['promo_text'] = array("name" => $shortcodeName, "handler" => $gt3_compileShortcodeUI);
