...
function gt3_showPortCategoryWithAjax($postid = "")
{
    if (!isset($term_list)) {
        $term_list = '';
    }
    ...
    if ($count > 0) {
        ...
        $term_list .= '<a href="#filter" data-option-value="*">' . ((gt3_get_theme_option("translator_status") == "enable") ? gt3_get_text("translator_portfolio_all") : __('All','theme_localization')) . '</a>
...
function gt3_showPortCategoryWithoutAjax($postid = "")
{
    if (!isset($term_list)) {
        $term_list = '';
    }
    ...
    if ($count > 0) {
        ...
        $term_list .= '<a href="'.$permalink.'" data-option-value="*">' . ((gt3_get_theme_option("translator_status") == "enable") ? gt3_get_text("translator_portfolio_all") : __('All','theme_localization')) . '</a>
        <div class="filter_fadder"></div>
		</li>';
        $termcount = count($terms) ;
        if (is_array($terms)) {
    ...
    <div id="comment-<?php comment_ID(); ?>" class="stand_comment">
        ...
        <div class="thiscommentbody">
            <div class="comment_info">
                <span class="author_name"><?php echo ((gt3_get_theme_option("translator_status") == "enable") ? gt3_get_text("posted_by") : __('Posted by','theme_localization')); ?> <?php printf('%s', get_comment_author_link()) ?> <?php edit_comment_link('(Edit)', '  ', '') ?></span>
                <span class="date"><?php printf('%1$s at %2$s', get_comment_date("d M Y"), get_comment_time()) ?></span>
                <?php comment_reply_link(array_merge($args, array('before' => ' <span class="comments">', 'after' => '</span>', 'depth' => $depth, 'reply_text' => (gt3_get_theme_option("translator_status") == "enable") ? gt3_get_text("translator_reply_value") : __('Reply','theme_localization'), 'max_depth' => $max_depth_comment))) ?>
            </div>
            <?php if ($comment->comment_approved == '0') : ?>
            <p><em><?php (gt3_get_theme_option("translator_status") == "enable") ? gt3_the_text("translator_awaiting_moder_value") : _e('Your comment is awaiting moderation.','theme_localization'); ?></em></p>
            <?php endif; ?>
            <?php comment_text() ?>
        </div>
        <div class="clear"></div>
...
#Custom paging
function gt3_get_pagination($range = 10, $type = "")
{
...
#Send feedback
function gt3_sendFeedback($feedback_email, $feedback_msg, $feedback_name, $subject)
{
    ...
    $admin_email = gt3_get_theme_option('contacts_to');
    if ($subject == "" || $subject == "Subject") {
        $subject = ((gt3_get_theme_option("translator_status") == "enable") ? gt3_get_text("contacts_subject") : __('[Website] Contact Form','theme_localization'));
    }
    $message = "
	<html>
	<head>
...
function testimonials_save_postdata($post_id)
{
    ...
    if (isset($_POST['testimonials_author'])) {
        $testimonials_author = esc_attr($_POST['testimonials_author']);
        update_post_meta($post_id, "testimonials_author", $testimonials_author);
    }

    if (isset($_POST['testimonials_position'])) {
        $testimonials_position = esc_attr($_POST['testimonials_position']);
...
function partners_save_postdata($post_id)
{
    ...
    if (isset($_POST)) {
        ...
        if (isset($_POST['partners_url'])) {
            $partners_url = esc_url($_POST['partners_url']);
...
function gt3_the_breadcrumb() {
    $showOnHome = 1;
    $delimiter = '';
    $home = ((gt3_get_theme_option("translator_status") == "enable") ? gt3_get_text("translate_home") : __('Home','theme_localization'));
    $showCurrent = 1;
