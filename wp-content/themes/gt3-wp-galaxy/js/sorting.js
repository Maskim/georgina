	...
	jQuery('.btn_load_more').click(function(){
		...
		if (now_load > 0) {
			if (addon_options.type == 0) {
				...
				for (i = i_start-1; i < i_start+now_load-1; i++) {
					loaded_object = loaded_object + '<div data-category="'+ addon_options.items[i].category +'" class="'+ addon_options.items[i].category +' element added"><div class="filter_img portfolio_item hided"><a href="'+ addon_options.items[i].url +'"><img src="'+ addon_options.items[i].src +'" alt="" width="570" height="570"><div class="gallery_fadder"></div><div class="post_type_ico"><svg class="iconbox-svg" viewBox="0 0 1000 1000"><rect width="1000" height="1000" /></svg><span class="post_type_' + addon_options.items[i].post_type + '"></span></div><!-- .post type --><div class="gallery_descr"><h5 class="gallery_title">'+ addon_options.items[i].title +'</h5><p>'+ addon_options.items[i].description +'</p></div></a></div></div><!-- .element -->';
